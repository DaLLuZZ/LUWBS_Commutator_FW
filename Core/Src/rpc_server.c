...
static osThreadId_t rpcTaskHandle;
static uint32_t rpcTaskBuffer[1024];
static StaticTask_t rpcTaskControlBlock;
static const osThreadAttr_t rpcTask_attributes = {
    .name = "RPC",
    .cb_mem = &rpcTaskControlBlock,
...
udp_recv_fn udp_echo_recv(void *arg, struct udp_pcb *pcb, struct pbuf *p, struct ip_addr *addr, u16_t port)
{
    if (p != NULL)
    {
        DBG_PRINTF("LXI discovery request received");
...
void rpc_server_thread(void *arg)
{
    ...
    while (1)
	{
        osDelay(500); // some delay!
    }
}

void rpc_server_init(void)
{
	rpcTaskHandle = osThreadNew(rpc_server_thread, NULL, &rpcTask_attributes);
}
